import pickle
from typing import Any

import gymnasium as gym
import numpy as np

import babybench.utils as bb_utils


class Eval:
    def __init__(self, env: gym.Env, render: bool, save_dir: str) -> None:
        self._env: gym.Env = env
        self._render: bool = render
        self._save_dir: str = save_dir
        self._images: list[np.ndarray] = []

    def reset(self) -> None:
        self._images = []
        self._init_track()

    def _eval_logs(self, logs: list[dict[str, Any]]) -> None:
        raise NotImplementedError

    def eval_logs(self) -> None:
        try:
            with open(f'{self._save_dir}/logs/training.pkl', 'rb') as f:
                logs = pickle.load(f)
        except FileNotFoundError:
            print('Training logs not found -- make sure to use the correct save_dir in the config')
            return

        score = self._eval_logs(logs)
        print(f'Preliminary training score: {score}')

    def _init_track(self) -> None:
        self._trajectories: dict[str, list] = {
            'qpos': [],
            'info': []
        }

    def track(self, info: dict[str, Any]) -> None:
        self._trajectories['qpos'].append(self._env.data.qpos.copy())
        self._trajectories['info'].append(info)

    def _render_image(self) -> None:
        self._images.append(bb_utils.evaluation_img(self._env))

    def eval_step(self, info: dict[str, Any]) -> None:
        self.track(info)

        if self._render:
            self._render_image()

    def end(self, episode: int = 0) -> None:
        # Store trajectories for submission
        with open(f'{self._save_dir}/logs/evaluation_{episode}.pkl', 'wb') as f:
            pickle.dump(self._trajectories, f, -1)

        # Store videos for submission
        if self._render:
            bb_utils.evaluation_video(
                images=self._images,
                save_name=f'{self._save_dir}/videos/evaluation_{episode}.avi'
            )


class EvalSelfTouch(Eval):
    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)

    def _eval_logs(self, logs: list[dict[str, Any]]) -> float:
        # Count total unique touches for left and right hands
        # during last 10000 episodes logged
        n_episodes = min(10001, len(logs))
        right_touches = np.array([])
        left_touches = np.array([])

        for ep in range(1, n_episodes):
            right_touches = np.unique(
                np.concatenate(
                    (right_touches, logs[-ep]['right_hand_touches']),
                    axis=0
                )
            )
            left_touches = np.unique(
                np.concatenate(
                    (left_touches, logs[-ep]['left_hand_touches']),
                    axis=0
                )
            )

        # Maximum of 34 geoms touched per hand
        score = (len(right_touches) + len(left_touches)) / (34 * 2)
        return score

    def _render_image(self) -> None:
        self._images.append(
            bb_utils.evaluation_img(self._env, up='touches_with_hands')
        )


class EvalHandRegard(Eval):
    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)

    def _eval_logs(self, logs: list[dict[str, Any]]) -> float:
        # Average time looking at hands
        # during last 1000 episodes logged
        n_episodes = min(10001, len(logs))
        hand_in_view = 0
        steps = 0

        for ep in range(1, n_episodes):
            hand_in_view += logs[-ep]['right_eye_right_hand']
            hand_in_view += logs[-ep]['left_eye_right_hand']
            hand_in_view += logs[-ep]['right_eye_left_hand']
            hand_in_view += logs[-ep]['left_eye_left_hand']
            steps += logs[-ep]['steps']

        score = hand_in_view / (4 * steps)
        return score

    def _render_image(self) -> None:
        self._images.append(
            bb_utils.evaluation_img(self._env, up='binocular')
        )


EVALS = {
    'none': Eval,
    'self_touch': EvalSelfTouch,
    'hand_regard': EvalHandRegard,
}
